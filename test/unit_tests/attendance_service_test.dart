...
void main() {
  ...
  group('AttendanceService', () {
    test('saveAttendanceRecord saves a record', () async {
      ...
      when(mockDatabaseService.saveAttendanceRecord(testRecord))
          .thenAnswer((_) async => 1); // Simulate successful insert

      await attendanceService.saveAttendanceRecord(testRecord);

      verify(mockDatabaseService.saveAttendanceRecord(testRecord))
          .called(1);
    });

    test('getAllAttendanceRecords returns all records', () async {
      final records = [
        AttendanceRecord(id: 1, employeeId: 'emp1', employeeName: 'Name1', timestamp: DateTime.now(), status: 'clockIn', isSynced: false),
        AttendanceRecord(id: 2, employeeId: 'emp2', employeeName: 'Name2', timestamp: DateTime.now(), status: 'clockOut', isSynced: true),
      ];

      // Mock the select statement
      when(mockDatabaseService.getAllAttendanceRecords())
    ...
    test('getUnsyncedAttendanceRecords returns only unsynced records', () async {
      ...
      when(mockDatabaseService.getUnsyncedAttendanceRecords())
    ...
    test('markAsSynced updates record status', () async {
      final recordToSync = AttendanceRecord(id: 1, employeeId: 'emp1', employeeName: 'Name1', timestamp: DateTime.now(), status: 'clockIn', isSynced: false);

      // Mock the update operation
      when(mockDatabaseService.markAsSynced(recordToSync))
